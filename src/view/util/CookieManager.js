...
export class CookieManager {
	...
	static executeOnce(callBack, cookieName, domain, a = []) {
		const argc = a.length;
		if (argc < 3) {
		...
		callBack.apply(argc > 3 ? a[1] : null, argc > 4 ? Array.prototype.slice.call(a, 2, argc - 2) : []);
		document.cookie = `${encodeURIComponent(cookieName)}=1; expires=Fri, 31 Dec 9999 23:59:59 GMT${domain || !a[argc - 1] ? '; path=/' : ''}`;
		return true;
	}
